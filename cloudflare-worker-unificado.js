  ...
  async fetch(request, env) {
    ...
    async function enviarTelegram(texto) {
      if (!env.TELEGRAM_BOT_TOKEN) {
        return { ok: false, status: 500, erro: "TELEGRAM_BOT_TOKEN não configurado." };
      }
      if (!env.TELEGRAM_CHAT_ID) {
        return { ok: false, status: 500, erro: "TELEGRAM_CHAT_ID não configurado." };
      }
      ...
      try {
        ...
        if (!resposta.ok || !dados.ok) {
          return {
            ok: false,
            status: 502,
            erro: "Erro ao enviar para o Telegram.",
            telegram: {
              ok: dados.ok ?? false,
              error_code: dados.error_code ?? null,
              description: dados.description ?? "Sem descrição"
            }
          };
        }
    ...
    async function enviarPlanilha(dados) {
      try {
        ...
        if (resposta.ok && retorno && retorno.sucesso === true) {
          return { ok: true, resposta: retorno };
        }
    ...
    if (request.method === "POST" && url.pathname === "/telegram/oracao") {
      try {
        ...
        if (!nome || !pedido) {
          return respostaJson({ sucesso: false, mensagem: "Nome e pedido são obrigatórios." }, 400);
        }

        const telegram = await enviarTelegram(
          `🙏 NOVO PEDIDO DE ORAÇÃO\n\n👤 Nome: ${nome}\n\n🙏 Pedido:\n${pedido}\n\n⛪ Igreja Batista Nova Família`
        );
        if (!telegram.ok) {
          return respostaJson({ sucesso: false, mensagem: telegram.erro, telegram: telegram.telegram || null }, telegram.status);
        }
    ...
    if (request.method === "POST" && url.pathname === "/telegram/visita") {
      try {
        ...
        if (!nome || !telefone || !data || !hora || !endereco) {
          return respostaJson({ sucesso: false, erro: "Preencha todos os campos obrigatórios." }, 400);
        }

        const telegram = await enviarTelegram(
          `🏠 NOVA SOLICITAÇÃO DE VISITA PASTORAL\n\n👤 Nome: ${nome}\n📞 Telefone / WhatsApp: ${telefone}\n📅 Melhor dia: ${data}\n🕒 Melhor horário: ${hora}\n📍 Endereço: ${endereco}\n📝 Motivo: ${motivo || "Não informado"}\n\n⛪ Igreja Batista Nova Família`
        );
        if (!telegram.ok) {
          return respostaJson({ sucesso: false, erro: telegram.erro, telegram: telegram.telegram || null }, telegram.status);
        }
    ...
    if (request.method === "POST" && url.pathname === "/telegram/visitante") {
      try {
        ...
        if (!nome || !telefone || !primeiraVez || !informacoes) {
          return respostaJson({ sucesso: false, mensagem: "Preencha todos os campos obrigatórios." }, 400);
        }
