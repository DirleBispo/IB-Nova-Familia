(function(){
  ...
  async function openDashboard(){
    ...
    const oracoes=o.data||[],visitas=v.data||[],visitantes=vi.data||[],servir=s.data||[],pessoas=p.data||[];
    const novos=oracoes.filter(x=>x.status==='novo');
    const visitasPend=visitas.filter(x=>['solicitada','agendada'].includes(x.status));
    const visitantesNovos=visitantes.filter(x=>x.status==='novo');
    const servirNovos=servir.filter(x=>x.status==='novo');
    ...
    const visitanteHtml=visitantes.slice(0,12).map(x=>`<div class="pastoral-item"><div><b>${escv(x.nome)}</b><small>${fmt(x.criado_em)} · ${escv(x.status)}</small><p>${escv(x.telefone)}${x.observacao?' · '+escv(x.observacao):''}</p></div><div class="pastoral-actions">${x.status!=='em_contato'?`<button onclick="ibnfSetPastoralStatus('visitantes','${x.id}','em_contato')">Em contato</button>`:''}${x.status!=='concluido'?`<button class="done" onclick="ibnfSetPastoralStatus('visitantes','${x.id}','concluido')">Concluir</button>`:''}</div></div>`).join('');
    const servirHtml=servir.slice(0,12).map(x=>`<div class="pastoral-item"><div><b>${escv(x.nome)}</b><small>${fmt(x.criado_em)} · ${escv(x.status)}</small><p>${escv(x.telefone)}${x.area?' · Área: '+escv(x.area):''}${x.observacao?' · '+escv(x.observacao):''}</p></div><div class="pastoral-actions">${x.status!=='em_contato'?`<button onclick="ibnfSetPastoralStatus('quero_servir','${x.id}','em_contato')">Em contato</button>`:''}${x.status!=='concluido'?`<button class="done" onclick="ibnfSetPastoralStatus('quero_servir','${x.id}','concluido')">Concluir</button>`:''}</div></div>`).join('');
    const html=`<div class="pastoral-summary"><div><b>${novos.length}</b><span>Orações novas</span></div><div><b>${visitasPend.length}</b><span>Visitas pendentes</span></div><div><b>${visitantesNovos.length}</b><span>Novos visitantes</span></div><div><b>${servirNovos.length}</b><span>Querem servir</span></div><div><b>${revisar.length}</b><span>Cadastros a revisar</span></div><div><b>${upcoming.length}</b><span>Aniversários 7 dias</span></div></div><div class="pastoral-section"><h3>Pedidos de oração</h3>${prayerHtml||'<div class="empty"><div>Nenhum pedido.</div></div>'}</div><div class="pastoral-section"><h3>Solicitações de visita</h3>${visitHtml||'<div class="empty"><div>Nenhuma solicitação.</div></div>'}</div><div class="pastoral-section"><h3>Novos visitantes</h3>${visitanteHtml||'<div class="empty"><div>Nenhum visitante novo.</div></div>'}</div><div class="pastoral-section"><h3>Quero servir</h3>${servirHtml||'<div class="empty"><div>Nenhum interesse registrado.</div></div>'}</div><div class="pastoral-section"><h3>Cadastros pendentes</h3>${revisar.map(x=>`<div class="pastoral-simple"><b>${escv(x.nome)}</b><span>Revisar cadastro</span></div>`).join('')||'<div class="empty"><div>Nenhuma pendência.</div></div>'}</div><div class="pastoral-section"><h3>Aniversários nos próximos 7 dias</h3>${upcoming.map(x=>`<div class="pastoral-simple"><b>${escv(x.nome)}</b><span>${x.next.toLocaleDateString('pt-BR',{day:'2-digit',month:'2-digit'})}</span></div>`).join('')||'<div class="empty"><div>Nenhum aniversário próximo.</div></div>'}</div>`;
