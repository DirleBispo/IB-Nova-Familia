(function(){
  ...
  async function setStatus(table,id,status){const {error}=await client.from(table).update({status}).eq('id',id);if(error){alert(error.message);return}if(window.ibnfRefreshPastoralHome)window.ibnfRefreshPastoralHome();openDashboard(currentTab)}
  async function removeItem(table,id){if(!confirm('Deseja realmente excluir esta solicitação? Esta ação não poderá ser desfeita.'))return;const {error}=await client.from(table).delete().eq('id',id);if(error){alert(error.message);return}if(window.ibnfRefreshPastoralHome)window.ibnfRefreshPastoralHome();openDashboard(currentTab)}
  window.ibnfSetPastoralStatus=setStatus;
  window.ibnfDeletePastoralItem=removeItem;
  window.ibnfOpenPastoralSection=(tab)=>openDashboard(tab||'resumo');
  ...
  async function openDashboard(tab='resumo'){
    currentTab=tab;
    ...
    window.openPanel('Painel Pastoral',tabs+(sections[tab]||sections.resumo));
  }
  window.showView=function(view){if(view==='pastoral'){openDashboard('resumo');return}return previousShowView(view)};
})();