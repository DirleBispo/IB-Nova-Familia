(function(){
  ...
  function abrirCadastro(){
    ...
    form.onsubmit=async e=>{
      ...
      const {data,error}=await sb.auth.signUp({email,password:senha,options:{data:{nome,telefone},emailRedirectTo:location.origin+'/'} });
      ...
      form.reset();
      fb.innerHTML=`<div class="success"><b>Cadastro recebido.</b><br>${data.session?'Sua conta foi criada e está aguardando aprovação da liderança.':'Confira seu e-mail para confirmar a conta. Depois disso, ela ficará aguardando aprovação da liderança.'}</div>`;
    };
  }
