...
alter table public.evento_destaque drop constraint if exists evento_destaque_id_check;
create sequence if not exists public.evento_destaque_id_seq;
select setval('public.evento_destaque_id_seq',greatest(coalesce((select max(id) from public.evento_destaque),1),1));
alter table public.evento_destaque alter column id set default nextval('public.evento_destaque_id_seq');
grant usage, select on sequence public.evento_destaque_id_seq to authenticated;
...
drop policy if exists "evento destaque administradores excluem" on public.evento_destaque;
create policy "evento destaque administradores excluem" on public.evento_destaque for delete to authenticated using (
  exists(select 1 from public.perfis p where p.id=auth.uid() and p.ativo=true and
    (p.perfil in ('pastor','admin','secretaria') or coalesce((p.permissoes->>'agenda')::boolean,false)))
);
grant delete on public.evento_destaque to authenticated;
