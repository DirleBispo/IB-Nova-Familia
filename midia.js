(function(){
  ...
  async function openMedia(message=''){
    ...
    items=(filesResult.data||[]).filter(item=>item.tipo!=='foto');
    const cards=await Promise.all(items.map(card));
    window.openPanel('Mídia',`${message}<div class="social-intro"><span class="section-kicker">Comunicação</span><h3>Equipe de Mídia</h3><p>Equipe, documentos e vídeos da comunicação da IB Nova Família. Os álbuns ficam na Galeria de Fotos.</p></div><div class="media-section-head"><h3>Integrantes</h3>${manager?'<button class="secondary-action" id="mediaManageTeam">Gerenciar equipe</button>':''}</div><div class="media-team">${memberRows(teamResult.data||[])}</div><div class="media-section-head"><h3>Documentos e vídeos</h3></div>${manager?'<div class="media-toolbar"><button class="primary" id="mediaAddLink">+ Adicionar documento ou vídeo</button></div>':''}<div class="media-grid">${cards.join('')||'<div class="media-empty">Nenhum documento ou vídeo publicado ainda.</div>'}</div>`);
  ...
  function openLink(){
    if(!manager)return;
    window.openPanel('Adicionar arquivo por link',`<form class="media-form" id="mediaLinkForm"><label>Tipo<select name="tipo" required><option value="documento">Documento ou PDF</option><option value="video_link">Vídeo</option></select></label><label>Título<input name="nome" maxlength="100" required placeholder="Ex.: Tutorial da plataforma"></label><label>Descrição<textarea name="descricao" maxlength="300" placeholder="Data ou informações sobre o arquivo"></textarea></label><label>Link público do arquivo no Google Drive ou YouTube<input name="url" type="url" required placeholder="https://..."></label><div class="media-limit">Os álbuns de fotos agora são adicionados diretamente na Galeria de Fotos.</div><button class="primary">Publicar link</button><button type="button" class="secondary-action" id="mediaCancel">Cancelar</button><div id="mediaFeedback"></div></form>`);
